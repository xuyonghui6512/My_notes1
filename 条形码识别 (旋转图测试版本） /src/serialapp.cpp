...
void SerialApp::send(SEND_FLAG send_flag, std::vector<float> num)
{
    ...
    if(SEND_ERR_UV == send_flag)
        buff[2] = 0x04;
    if(SEND_ASSIST_AIM == send_flag)//仅用这个
        buff[2] = 0x05;
...
//    std::cout << "1" << std::endl;
    ...
    buff[num.size()*3+4] = LAST_ONE;
    buff[num.size()*3+4+1] = LAST_TWO;
    ...
    for(size_t i = 0 ;i < received_length;i++)
    {
        switch (flag)
        {
            ...
            case 3:     ///类型
                ...
                {
                    temp_receive_flag = RECEIVE_ASSIST_AIM;
                    flag = 4;
                }
                else if(0x05 == buff[i])
                {
                    temp_receive_flag = RECEIVE_PITCH_ANGLE;//仅用这个？
                    flag = 4;
                }
                else
                    flag = 1;

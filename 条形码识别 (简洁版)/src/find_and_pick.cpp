...
//       waitKey(0);
    ...
    Mat CodeDetect::Find_Bar_Code_And_Rotate(Mat &_img)
    {
        ...
        Mat threshold_img;
        threshold(blur_img,threshold_img,225,255,CV_THRESH_BINARY);
        Mat kernel;
        kernel=getStructuringElement(MORPH_RECT,Size(33,11));
        Mat close_img;
        morphologyEx(threshold_img,close_img,MORPH_CLOSE,kernel);//闭运算
        Mat element=getStructuringElement(MORPH_RECT,Size(9,9));//得到自定义核
        erode(close_img,close_img,element);
        Mat element1=getStructuringElement(MORPH_RECT,Size(36,36));//得到自定义核

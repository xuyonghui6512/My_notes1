...
//       waitKey(0);
    ...
    bool CodeDetect::barcode_oriented(Mat &img,RotatedRect& findrect) {
        Mat srcImgA = img.clone();
        Mat srcImgB = mybar_code::Image_handle::turn90degree(srcImgA);
        Mat srcGrayA,srcGrayB;
        //对A图处理
        cvtColor(srcImgA, srcGrayA, CV_RGB2GRAY);

        Mat gradA_x, gradA_y;
        Sobel(srcGrayA, gradA_x, CV_32F, 1, 0, -1);//求x方向梯度
        Sobel(srcGrayA, gradA_y, CV_32F, 0, 1, -1);//y方向梯度
        Mat gradientA;
        subtract(gradA_x, gradA_y, gradientA);
        convertScaleAbs(gradientA, gradientA);//得到包含高水平梯度和低竖直梯度的图像区域
        Mat blurA_img;
        ...
        Mat gradB_x, gradB_y;
        Sobel(srcGrayB, gradB_x, CV_32F, 1, 0, -1);//求x方向梯度
        Sobel(srcGrayB, gradB_y, CV_32F, 0, 1, -1);//y方向梯度
        Mat gradientB;
        subtract(gradB_x, gradB_y, gradientB);
        convertScaleAbs(gradientB, gradientB);//得到包含高水平梯度和低竖直梯度的图像区域
        Mat blurB_img;
        blur(gradientB, blurB_img, Size(9, 9));//模糊
        //////////////////////////////
        Mat threshold_img,thresholdA_img,thresholdB_img,blur_img;
        threshold(blurA_img, thresholdA_img, 200, 255, CV_THRESH_BINARY);
        threshold(blurB_img, thresholdB_img, 200, 255, CV_THRESH_BINARY);
        Mat choosen=mybar_code::Image_handle::Best_Image_Choose(thresholdA_img,thresholdB_img,key);
        if(choosen.cols==thresholdA_img.cols)
        {
            blur_img=blurA_img.clone();
        } else
        {
            blur_img=blurB_img.clone();
        }
        ...
        while (timer--) {
            ...
            for(int i=0;i<threshold_img.rows-2;i=i+3)
            {
...
//         imshow("high_close_img3", close_img);
        ...
        ////////////////////////////////////////
        if(contours.size()>0&&place_rect.center.x>80&&
                place_rect.center.y>40&&place_rect.center.x<blur_img.cols-80
                &&place_rect.center.y<blur_img.rows-40) {
            int num, num1;
            ...
            for (int i = 0; i < contours.size(); i++) {
                if (max_Contour1.size() < contours[i].size()) {
                    max_Contour3=max_Contour2;
                    max_Contour2=max_Contour1;
...
//            cout<<"11111111111111111111111111 "<<findrect.size.area()<<endl;
            if(findrect.size.area()>50000||findrect.size.area()<12000)
                return false;
            return true;
        }
        else
...
///////////////////////////对找到的旋转矩形区域进行旋转矫正///////////////////////////////////////////////
    bool CodeDetect::Rotate_and_Find_barcode(RotatedRect& findrect,Mat& input,Mat& output)
    {
        ...
        if(key==1)
        {
            input=mybar_code::Image_handle::turn90degree(input);
        }
...
//           cout<<"这是二维码"<<endl;
            ...
            QRkey=(max_x-min_x+max_y-min_y)*2;
...
//
    ...
    bool CodeDetect::Find_QRCode_RotateRect(Mat& inputImg, Point2f oritented_pt,RotatedRect& output)
    {
        ...
        threshold(srcGray,threshold_img,100,255,CV_THRESH_BINARY);
        imshow("二值",threshold_img);
        waitKey();
        Mat element=getStructuringElement(MORPH_RECT,Size(23,23));//得到自定义核
        erode(threshold_img,threshold_img,element);
       // imshow("111",threshold_img);
...
//        drawContours(inputImg,contours2,findnum,Scalar(0,0,125),5);
    ...
    bool CodeDetect::Rotate_and_Find_QRcode(RotatedRect& findrect,Mat& input,Mat& output)
    {
        Point2f rect_dst[4],rect_rotated[4];
        findrect.points(rect_dst);
        float  angle=findrect.angle;
